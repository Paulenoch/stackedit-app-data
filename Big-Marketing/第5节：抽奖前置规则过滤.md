...
# DefaultRaffleStrategy 的“逐行”执行流程
### 0）拿到“规则实现”的目录表

-   通过规则工厂拿到 `Map<规则code, 规则实现>`，后面按 code 去取对应规则并执行。  
    （这个目录来自工厂的 `openLogicFilter()`，工厂在启动时已把带 `@LogicStrategy` 的规则都注册好了。）
    

### 1）黑名单优先：如果配置里包含黑名单，就先跑它

-   从 `logics`（策略里配置的规则数组）里找是否包含 `rule_blacklist`；这里用的是 `contains`。
    
-   命中后，按 code 取出黑名单规则实现，拼好 `RuleMatterEntity`（带 `userId/strategyId/ruleModel`），调用规则的 `filter(...)`。如果返回的不是 **ALLOW**，就**立刻返回**（让这条规则“接管”后续）。
    

> 小提示：这一步等于给了“黑名单一票否决权”。

### 2）其次，顺序跑剩余规则

-   把黑名单之外的规则收集成 `ruleList`（注意这里用的是 `!equals` 来排除黑名单）。
    
-   依次取出每个 `ruleModel` 对应的实现，构造 `RuleMatterEntity` 调用 `filter(...)`；如果某条规则返回的也不是 **ALLOW**，就**立刻返回**该结果（谁先接管听谁的）。
    
-   如果所有规则都 **ALLOW**，方法就把最后一次规则结果返回（当前代码里可能是 `null`，见“坑点”）。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA4MzcxMDgzMCw5Njg2MDM1MzgsMTgxOD
c0NzI0NCw5NzkyMjc2ODAsLTIwODg3NDY2MTJdfQ==
-->