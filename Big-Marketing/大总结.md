...
在用户进行每日签到返利的业务中，为了保证一个用户一天只能签到一次，系统采用了**当前日期**作为 `outBusinessNo`。
在用户使用积分兑换抽奖次数（SKU）的场景下，每次兑换都应该是一个全新的、唯一的订单。因此，系统采用了**随机生成的字符串**作为 `outBusinessNo`。
...
当用户点击抽奖按钮时，系统会为这次抽奖行为创建一个抽奖单 (`UserRaffleOrder`)。

-   **代码实现**： 在 `AbstractRaffleActivityPartake.java` 的 `createOrder` 方法中，系统在创建新的抽奖单之前，会先调用 `activityRepository.queryNoUsedRaffleOrder` 方法，查询该用户在此活动下是否已经存在“创建状态（create）”且“未使用（not_used）”的抽奖单。
    
    -   如果存在，就直接返回这个已存在的抽奖单，而不会创建新的。
        
    -   这样即使用户的抽奖请求被重复发送，也只会对应同一个抽奖单，不会重复扣减用户的抽奖资格。
        

### 2. 发放奖励时：利用订单状态更新的原子性

在抽奖完成后，需要为用户记录中奖信息并发放奖励。这个环节的幂等性至关重要，防止用户重复领奖。

-   **代码实现**：
    
    1.  **生成中奖记录**：当用户中奖后，系统会创建一条 `user_award_record` 中奖记录，这条记录包含了本次抽奖的 `orderId`。
        
    2.  **更新抽奖单状态**：最关键的一步是，在同一个数据库事务内，系统会调用 `userRaffleOrderDao.updateUserRaffleOrderStateUsed` 方法，将 `user_raffle_order` 表中对应 `orderId` 的订单状态从 `create` 更新为 `used`。
        
    3.  **原子性保证**：这个更新操作是带有条件的 (`where ... and order_state = 'create'`)。这意味着只有当订单状态是 `create` 时，更新才会成功。如果因为重试导致重复处理同一个已中奖的订单，由于该订单的状态已经被更新为 `used`，后续的更新操作将不会执行任何操作（返回更新条数为0）。
        
    4.  **事务控制**：在 `AwardRepository.java` 的 `saveUserAwardRecord` 方法中，写入中奖记录和更新抽奖单状态这两个操作被包裹在一个事务中。如果更新抽奖单状态失败（比如已经被更新过），整个事务会回滚，从而保证了不会产生错误的中奖记录。
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDUzODE5NjYwLC05OTc4Mzk3MTYsMjAxOT
Y3NTkzOCwxMTM0NDE3MTA4LDEwMjkyODU0MywtMTQyNDUyMDE3
MSwtMjA3MzIwNTg0NiwxNjU1MDc2NjIzLDIwMjU2ODk0MTYsLT
c5NTU0MDAwLC0zNDc4NTA4MywxOTY2NzA0NDI4LDUwNTMwOTg0
LC0xMjk1NTI1MTQ3LDE1NjkxMTM3NjFdfQ==
-->