...
1.  **获取 PID**
    ...
    -   当一个生产者启动时，它会向 Kafka 集群的一个 Broker 发送一个请求，申请一个唯一的 `Producer ID` (PID)。
        
    -   这个 PID 对这个生产者实例是唯一的。如果生产者重启，它会获取一个新的 PID。
        
2.  **发送带有序列号的消息**
    
    -   生产者在发送消息时，不再是简单地发送消息内容。它会为发送到**每一个分区**的消息都维护一个从 0 开始递增的**序列号 (Sequence Number)**。
        
    -   例如，生产者要向 `Topic-A` 的 `Partition-0` 发送三条消息，它会这样做：
        
        -   第一条消息：`(PID, Partition-0, Seq-0)`
            
        -   第二条消息：`(PID, Partition-0, Seq-1)`
            
        -   第三条消息：`(PID, Partition-0, Seq-2)`
            
    -   这个 `(PID, Partition, Sequence Number)` 的组合就成了每条消息的唯一标识。
        
3.  **Broker 的验证与处理**
    
    -   Broker 会在内存中为每个 `(PID, Partition)` 组合缓存一个最新的序列号。
        
    -   当 Broker 收到一条消息时，它会进行如下检查：
        
        -   **情况一：收到的序列号 `正好` 是 `Broker 期望的下一个序列号`**
            
            -   比如，Broker 缓存中 `(PID=1, Partition=0)` 的最新序列号是 `5`。现在收到一条序列号为 `6` 的消息。
                
            -   这说明是正常的新消息，Broker 会接收它，并将缓存中的序列号更新为 `6`。
                
            -   `收到的 Seq (6) == 缓存的 Seq (5) + 1`
                
        -   **情况二：收到的序列号 `小于或等于` Broker 缓存的序列号**
            
            -   比如，Broker 缓存的序列号是 `5`，但又收到一条序列号为 `5` 或者 `4` 的消息。
                
            -   这说明这条消息是**重复**的（由于生产者的重试导致）。
                
            -   Broker 会**直接丢弃**这条消息，但依然会像成功处理一样返回一个 ACK 给生产者。这样生产者就认为发送成功了，不会再重试。
                
            -   `收到的 Seq (<=5) <= 缓存的 Seq (5)`
                
        -   **情况三：收到的序列号 `大于` Broker 期望的下一个序列号**
            
            -   比如，Broker 缓存的序列号是 `5`，但收到一条序列号为 `7` 的消息。
                
            -   这说明中间的消息（序列号为 6 的）丢失了。
                
            -   Broker 会判断这是一个乱序的请求，会拒绝该消息，并返回一个 `OutOfOrderSequenceException` 异常给生产者。生产者在收到这个异常后会进行相应的处理（通常是报错）。
                
            -   `收到的 Seq (7) > 缓存的 Seq (5) + 1`
                

通过这个机制，Kafka 完美地保证了在单个生产者会话中，发送到单个分区的消息不会因为重试而重复。

### 4. 幂等性的限制

需要注意的是，Kafka 的幂等性有其作用范围：

1.  **单分区**：幂等性只能保证单个分区内的消息写入是幂等的，不能跨多个分区。
    
2.  **单会话**：幂等性只在单个生产者会话内有效。如果生产者进程重启，会获取新的 PID，序列号也会从 0 开始，无法和重启前的状态衔接。
