...
`cs_inhouse_user_all_ctl_live` 总量约 7.55 GB，`string` 占约 87.0%，最大头非常集中：

-   `1-user:tag:v2:live:`：约 5.87 GB / 1.01 亿 key，几乎占整个集群的 72% 左右，是首要目标。
    
-   `1-match_`：约 1.27 GB / 890 万 key。
    
-   `1-user_info_str_live:`：约 388 MB / 104 万 key。这个前缀虽然没前两个大，但从 top key 可以直接看到**同一个 user 同时存在带 region 和不带 region 的两份缓存**，很像重复缓存。

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExMjY4ODk4ODksOTM0Mzg5NjIyXX0=
-->